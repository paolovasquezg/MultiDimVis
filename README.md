...
|---|---|
| **RadViz** (T1 · T3) | Places all 9 features as anchors around a circle — the pull toward each anchor shows which features dominate a group of tracks, making decade-level patterns easy to spot. |
| **Star Coordinates** (T3) | Axes are draggable, so you can manually rotate and scale dimensions until decades separate — direct exploration of which audio features drive the shift over time. |
| **Parallel Coordinates** (T2) | Brushing each axis isolates specific feature ranges, so you can immediately see which combinations show up consistently in high or low popularity tracks. |
| **PCA Projection** (T1 · T2) | Reduces 9 dimensions down to 2. With decade coloring you can see if tracks group by era; switching to popularity coloring shows whether popular tracks occupy a distinct region. |
